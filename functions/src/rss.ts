...
const STORAGE_URL_PREFIX = "https://storage.googleapis.com/primary-climatechange";
const STATIC_STORAGE_URL_PREFIX = "https://storage.googleapis.com/static-climatechange";
const writeFeed = async (xml: string): Promise<void> => {
  const file = await storage.bucket("static-climatechange").file("rss.xml");
  await file.save(xml);
